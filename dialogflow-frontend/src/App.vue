<template>
  <main id="app">
          ...
          <RichComponent v-for="(component, component_id) in message.queryResult.fulfillmentMessages" :key="component_id" :fullwidth="component.carouselSelect !== undefined || component.rbmCarouselRichCard !== undefined || component.payload && component.payload.richContent !== undefined">
            ...
            <div v-if="component.rbmText">
              ...
              <div v-for="(suggestion, suggestion_id) in component.rbmText.rbmSuggestion" :key="suggestion_id">
                <RichCardButton
                  ...
                  @click="send({text: suggestion.reply.text.postbackData})"
                />
            ...
            <RichList
              ...
              <RichListItem
                ...
                :image-uri="item.image.imageUri"
                :image-title="item.image.accessibilityText"
                @select="send({text: item.info.key})"
              />
            </RichList>
            ...
            <RichComponent v-for="(component, component_id) in message.queryResult.webhookPayload.google.systemIntent" :key="component_id" :fullwidth="component.carouselSelect !== undefined">
              ...
              <RichList
                ...
                <RichListItem
                  ...
                  :image-uri="item.image.url"
                  :image-title="item.image.accessibilityText"
                  @select="send({text: item.optionInfo.key})"
                />
              </RichList>
    ...
    <ChatField ref="input" @submit="send" @listening="stop_feedback" @typing="stop_feedback">
      ...
      -->
      <RichSuggesion
        v-for="(suggestion, suggestion_id) in suggestions.textSuggestions"
        :key="'text-' + suggestion_id"
        :title="suggestion"
        @select="send({text: suggestion})"
      />
      ...
      -->
      <RichSuggesion
        v-for="(suggestion, suggestion_id) in suggestions.linkSuggestions"
  ...
  computed: {
    ...
    suggestions () {
      if (this.messages.length > 0) {
        ...
        for (const component in lastMessage.queryResult.fulfillmentMessages) {
          if (lastMessage.queryResult.fulfillmentMessages[component].suggestions) textSuggestions.push(...lastMessage.queryResult.fulfillmentMessages[component].suggestions.suggestions.map(suggestion => suggestion.title))
          if (lastMessage.queryResult.fulfillmentMessages[component].linkOutSuggestion) linkSuggestions.push(lastMessage.queryResult.fulfillmentMessages[component].linkOutSuggestion)
          if (lastMessage.queryResult.fulfillmentMessages[component].quickReplies) textSuggestions.push(...lastMessage.queryResult.fulfillmentMessages[component].quickReplies.quickReplies)
          if (lastMessage.queryResult.fulfillmentMessages[component].payload && lastMessage.queryResult.fulfillmentMessages[component].payload.richContent) {
            lastMessage.queryResult.fulfillmentMessages[component].payload.richContent.forEach(stack => {
              const chips = stack.find(item => item.type === 'chips')
              if (chips) linkSuggestions.push(...chips.options)
            })
          }
        }

        /* Google Suggestions */
        if (lastMessage.queryResult.webhookPayload && lastMessage.queryResult.webhookPayload.google) {
          for (const component in lastMessage.queryResult.webhookPayload.google) {
            if (lastMessage.queryResult.webhookPayload.google[component].suggestions) textSuggestions.push(...lastMessage.queryResult.webhookPayload.google[component].suggestions.map(suggestion => suggestion.title))
            if (lastMessage.queryResult.webhookPayload.google[component].linkOutSuggestion) linkSuggestions.push(lastMessage.queryResult.webhookPayload.google[component].linkOutSuggestion)
          }
        }
